...
class ManageClinic extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      descriptionMarkdown: "",
      listClinics:[],
      deleteConfirmationModalOpen: false, // State để kiểm soát việc hiển thị modal xác nhận xóa
      clinicIdToDelete: null, // Lưu clinicId mà người dùng muốn xóa
      clinicNameToDelete: null, // Lưu clinicId mà người dùng muốn xóa
    };
  }
  ...
  handleDeleteClinic = async (clinicId) => {
    this.setState({ deleteConfirmationModalOpen: false }); // Đóng modal khi xóa bắt đầu
    let { language } = this.props;
    let res = await deleteClinic({ id: clinicId });
    if (res && res.errCode === 0) {
      if (language === "en") {
  ...
  openDeleteConfirmationModal = (clinicId, clinicName) => {
    this.setState({ deleteConfirmationModalOpen: true, clinicIdToDelete: clinicId, clinicNameToDelete: clinicName });
  };

  // Hàm đóng modal xác nhận xóa
  closeDeleteConfirmationModal = () => {
    this.setState({ deleteConfirmationModalOpen: false, clinicIdToDelete: null, clinicNameToDelete: null });
  };
      ...
      <div className="manage-specialty-container">
                    ...
                    <tr>
                        ...
                        <td class="" colspan="2">
                              ...
                              <button
                                ...
                                onClick={() => this.openDeleteConfirmationModal(clinic.id, clinic.name)}
                              >
        ...
        {this.state.deleteConfirmationModalOpen && (
          <div className="modal-bg">
            <div className="modal">
              <h2>Bạn có chắc muốn xóa {this.state.clinicNameToDelete} ?</h2>
              <div className="btn-closeModal">
                <button onClick={this.closeDeleteConfirmationModal}>Hủy</button>
                <button onClick={() => this.handleDeleteClinic(this.state.clinicIdToDelete, this.state.clinicNameToDelete)}>Xác nhận</button>
              </div>
            </div>
          </div>
        )}
